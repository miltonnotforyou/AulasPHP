    ...
    <div class="row">
      ...
      <main class="ms-auto col-lg-10 px-md-4">
          ...
          if(isset($_GET['codigo_marca']) && $_GET['codigo_marca'] != '')          
        {
          ...
          $codigo = intval($_GET['codigo_marca']); // Recebe o código da marca via GET para buscar os dados e preencher o formulário de edição
          $sql = "SELECT * FROM marca WHERE codigo_marca = $codigo"; // Query para buscar a marca específica pelo código recebido via GET

          $query = mysqli_query($conexao, $sql); // Executa a query para buscar a marca específica
          $marca = mysqli_fetch_assoc($query); // mysqli_fetch_assoc() retorna os dados da marca como um array associativo, onde as chaves são os nomes das colunas do banco de dados
          ...
          <div class="card-body">
            <form action="acoes.php" method="post">
                ...
                <div class="col-12 mt-2">
                    ...
                    <textarea name="observacao" id="observacao" class="form-control" maxlength="100"><?php echo $marca['observacao']; ?></textarea>

                    <!-- Campo oculto para identificar a ação de cadastro de marca -->
                    <input type="hidden" name="editar" value="editar_marca"> 
                    <!-- Campo oculto para enviar o código da marca a ser editada -->
      ...
      // Nenhuma marca encontrada: exibe mensagem centralizada
